...
const sendPushToSubscription = async (subscription, payload) => {
    ...
    try {
        ...
        // 410 Gone = subscription expired/unsubscribed → delete it
...
// ─── Send push to list of user IDs ──────────────────────────────────────────
const sendPushToUsers = async (userIds, payload) => {
    if (!userIds || userIds.length === 0) return;

    PushSubscription.getByUserIds(userIds, async (err, subscriptions) => {
...
const sendPushToUser = async (userId, payload) => {
    PushSubscription.getByUserId(userId, async (err, subscriptions) => {
        if (err || !subscriptions.length) return;
        for (const sub of subscriptions) {
            await sendPushToSubscription(sub, payload);
        }
...
const buildEmailHtml = (title, message, taskId = null) => {
    ...
    <div style="font-family:Arial,sans-serif;direction:rtl;max-width:560px;margin:0 auto;background:#f8fafc;padding:24px;border-radius:12px;">
      <div style="background:#0f172a;padding:16px 24px;border-radius:8px 8px 0 0;">
        <h1 style="color:#fff;margin:0;font-size:20px;">TaskMaster Pro</h1>
      </div>
      <div style="background:#fff;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;">
        ...
        <p style="color:#94a3b8;font-size:12px;margin:0;">هذا البريد تم إرساله تلقائياً من نظام TaskMaster Pro</p>
      </div>
    </div>`;
};

// ─── High-level: notify user with push + email fallback ──────────────────────
/**
 * Send push notification + email fallback to a user
 * Email is sent when user has been inactive > 1 hour (likely offline)
 */
const notifyUser = async (userId, payload, { forceEmail = false } = {}) => {
    // Always send push
    await sendPushToUser(userId, payload);

    // Send email if forced (e.g. inactivity) or check last_login
    if (forceEmail) {
        db.query('SELECT email, name, last_login FROM user WHERE id = ?', [userId], async (err, results) => {
            ...
            if (!lastLogin || lastLogin < oneHourAgo) {
                await sendEmail(
                    user.email,
                    payload.title,
                    buildEmailHtml(payload.title, payload.body, payload.data?.taskId)
                );
            }
        });
    }
...
/**
 * Notify list of user IDs with push + optional email
 */
const notifyUsers = async (userIds, payload, options = {}) => {
    for (const uid of userIds) {
        await notifyUser(uid, payload, options);
    }
};
